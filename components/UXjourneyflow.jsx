...
function computeEdgeRoutes(graph, positions) {
  ...
  return graph.edges.map((edge) => {
    ...
    const fromAnchor = getAnchor(fromPos, fromNode, "bottom");
    ...
    if (!edge.isLoop) {
      const distance = Math.abs(toAnchor.y - fromAnchor.y);
      const curvature = clamp(distance * 0.36, 110, 300);

      const c1x = fromAnchor.x + spread;
      const c1y = fromAnchor.y + curvature;
      const c2x = toAnchor.x + spread;
      const c2y = toAnchor.y - curvature;

      const labelX = (fromAnchor.x + toAnchor.x) / 2;
      const labelY = (fromAnchor.y + toAnchor.y) / 2 + spread - 16;

      return {
        edge,
        d: `M ${fromAnchor.x} ${fromAnchor.y} C ${c1x} ${c1y}, ${c2x} ${c2y}, ${toAnchor.x} ${toAnchor.y}`,
        labelX,
        labelY,
        isLoop: false,
      };
    ...
    const bandKey = `${Math.min(fromLevel, toLevel)}|${Math.max(fromLevel, toLevel)}`;
    const lane = loopLaneByBand.get(bandKey) || 0;
    loopLaneByBand.set(bandKey, lane + 1);

    const span = Math.max(1, Math.abs(fromLevel - toLevel));
    const side = 190 + span * 34 + lane * 52;
    const rightX = Math.max(fromAnchor.x, toAnchor.x) + side;

    const startHookY = fromAnchor.y + 86;
    const endHookY = toAnchor.y - 86;

    const d = [
      `M ${fromAnchor.x} ${fromAnchor.y}`,
      `C ${fromAnchor.x} ${fromAnchor.y + 56}, ${rightX - 32} ${startHookY}, ${rightX} ${startHookY}`,
      `L ${rightX} ${endHookY}`,
      `C ${rightX - 32} ${endHookY}, ${toAnchor.x} ${toAnchor.y - 56}, ${toAnchor.x} ${toAnchor.y}`,
    ].join(" ");
    ...
    return {
      ...
      d,
      labelX: rightX + 4,
      labelY: (startHookY + endHookY) / 2 - 10,
      isLoop: true,
    };
  });
}
...
function estimateLoopBounds(graph, positions) {
  ...
  graph.edges.forEach((edge) => {
    ...
    const side = 190 + span * 34;

    minX = Math.min(minX, from.x, to.x);
    maxX = Math.max(maxX, from.x, to.x + side);
