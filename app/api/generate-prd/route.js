...
const PRD_AGENT_NAME =
  process.env.AGENT5I_PRD_AGENT_NAME ||
  "AI PRD Generator";

  console.log("PRD_AGENT_NAME:", PRD_AGENT_NAME);
...
function extractPrd(rawMessage) {
  if (rawMessage && typeof rawMessage === "object") {
    if (rawMessage.message) {
      const nested = extractPrd(rawMessage.message);
...
function parsePrdDocument(prdDocRaw) {
  if (prdDocRaw && typeof prdDocRaw === "object") {
    return prdDocRaw;
  }

  if (typeof prdDocRaw === "string") {
    const parsed = parseJsonWithFallback(prdDocRaw);
...
function pickPrdDocument(parsed) {
  if (!parsed || typeof parsed !== "object") {
    return null;
  }
...
function toRawPrdFallback(value) {
...
Gender: ${normalizeText(i.gender) || "N/A"}
Age: ${normalizeText(i.age) || "N/A"}
Location: ${normalizeText(i.location) || "N/A"}
Relationship Status: ${
          normalizeText(i.relationship_status) || "N/A"
        }
Title: ${normalizeText(i.title) || "N/A"}
Education: ${normalizeText(i.education) || "N/A"}`
  ...
  sections.push(
    `Interviews:\n${formatList(
      ...
      (iv, index) =>
        `Interview ${index + 1}:
Transcript: ${normalizeText(iv.transcript).slice(0, 1500)}|| "N/A"}
Persona Output:
${normalizeText(iv.persona_output).slice(0, 1500)}
Interview Outcome: ${
          normalizeText(iv.interview_outcome) || "N/A"
        }
...
Needs: ${normalizeText(row.needs) || "N/A"}`
  ...
  sections.push(
    `Generated Personas:\n${formatList(
      ...
      (row, index) => {
        const generated =
          tryParseJson(row.generated_output) ||
          row.generated_output;

        return `Generated Persona ${index + 1}:
Persona Name: ${normalizeText(row.persona_name) || "N/A"}
...
Negative Themes: ${normalizeText(row.negative_themes) || "N/A"}
Needs & Expectations: ${
          normalizeText(row.needs_expectations) || "N/A"
        }
Generated Output: ${normalizeText(generated) || "N/A"}`;
      }
    )}`
  );

  const iaParsed =
  tryParseJson(informationArchitecture?.ia);

let iaText = "N/A";

if (iaParsed) {
  iaText = JSON.stringify(
    iaParsed,
    null,
    2
  ).slice(0, 3000);
...
sections.push(
  ...
  const processFlowParsed =
  tryParseJson(processFlow?.process_flow);

let processFlowText = "N/A";

if (
  processFlowParsed?.nodes &&
  Array.isArray(processFlowParsed.nodes)
) {
  processFlowText =
    processFlowParsed.nodes
      .map(
...
export const POST = withAuth(async (request, _ctx, user) => {
  ...
  } = await validateBody(request, generatePRDSchema);

  if (validationError) {
    return validationError;
  }

  const { limited, retryAfterSec } =
    aiHeavyLimiter.check(String(user.userId));
  ...
  try {
    ...
    const [
      ...
      pool.request()
        .input("projectId", sql.Int, projectId)
        .query(`
          SELECT ie.interviewee_id, ie.persona_id,
                 ie.name, ie.gender, ie.age,
                 ie.location, ie.relationship_status,
                 ie.title, ie.education
          FROM intervieweess ie
          INNER JOIN personass p
            ON p.persona_id = ie.persona_id
      ...
      pool.request()
        ...
        .query(`
          SELECT iv.interview_id,
                 ...
                 iv.transcript,
                 iv.persona_output,
                 iv.interview_outcome,
                 iv.summary
          FROM interviewss iv
          INNER JOIN intervieweess ie
            ON ie.interviewee_id = iv.interviewee_id
          INNER JOIN personass p
            ON p.persona_id = ie.persona_id
          WHERE p.project_id = @projectId
      ...
      pool.request()
        ...
        .query(`
          ...
          INNER JOIN interviewss iv
            ON iv.interview_id = pi.interview_id
          INNER JOIN intervieweess ie
            ON ie.interviewee_id = iv.interviewee_id
          INNER JOIN personass p
            ON p.persona_id = ie.persona_id
    ...
    const projectBrief = buildProjectBrief({
      project,
      personas:
        personasResult.recordset || [],
      interviewees:
        intervieweesResult.recordset || [],
      interviews:
        interviewsResult.recordset || [],
      insights:
        insightsResult.recordset || [],
      problemStatement:
        problemStatementResult.recordset?.[0] ||
        null,
      generatedPersonas:
        generatedPersonasResult.recordset ||
        [],
      informationArchitecture:
        iaResult.recordset?.[0] || null,
      processFlow:
        processFlowResult.recordset?.[0] ||
        null,
    });
...
console.log("PASSWORD EXISTS:", !!PASSWORD);
    console.log("PRD PAYLOAD:", JSON.stringify(payload, null, 2));
...
console.log("================================");
    ...
    if (!response.ok) {
      logger.error(
        "POST /api/generate-prd upstream error",
        {
          status:
            response.status,
          preview:
    ...
    if (!data) {
      logger.error(
        "POST /api/generate-prd invalid upstream JSON",
        {
          preview:
            text.slice(0, 500),
        }
    ...
    const parsed =
      extractPrd(rawMessage);

    if (!parsed) {
      logger.error(
        "POST /api/generate-prd failed to parse agent message",
        {
          rawMessageType:
            typeof rawMessage,
        }
    ...
    if (!prdData) {
      logger.error(
        "POST /api/generate-prd non-JSON prd_document fallback",
        {
          prdDocRawType:
            typeof prdDocRaw,
        }
      ...
      return NextResponse.json({
        ...
        data: {
          prd: {
            ...
            note:
              "Unable to parse prd_document as strict JSON; showing raw content.",
          },
        },
      });
    }
