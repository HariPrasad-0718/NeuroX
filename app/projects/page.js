...
export default function ProjectsPage() {
  ...
  const [searchMode, setSearchMode] = useState("name"); // "name" | "domain"
              ...
              >
                ...
                <div className="p-6 relative">
                  ...
                  <div className="flex items-center gap-2 mb-4 flex-wrap">
                    <span className={`inline-flex text-xs px-3 py-1.5 rounded-full font-medium ${project.status === "Completed" ? "bg-emerald-500 text-white" : "bg-amber-500 text-white"}`}>
                      {project.status}
                    </span>
                    {project.domain && (
                      <span className="inline-flex text-xs px-3 py-1.5 rounded-full font-medium bg-white/20 text-white">
                        {project.domain}
                      </span>
                    )}
                  </div>
