...
const PROCESS_DISCOVERY_AGENT_NAME =
  process.env.AGENT5I_PROCESS_DISCOVERY_AGENT_NAME ||
...
function buildProcessDiscoveryPayload(
  projectName,
  projectDescription
) {
  return {
    username: AGENT5I_USERNAME,
    password: AGENT5I_PASSWORD,
    name: PROCESS_DISCOVERY_AGENT_NAME,
...
function extractMandatoryQuestions(data) {
  if (!data) return [];
  ...
  if (typeof data === "string") {
    ...
    if (parsed) {
      return extractMandatoryQuestions(parsed);
    }
  ...
  if (typeof data === "object") {
    if (data.mandatory_questions) {
      return extractMandatoryQuestions(data.mandatory_questions);
    }

    for (const value of Object.values(data)) {
      const result = extractMandatoryQuestions(value);
...
export async function enhancePersonaDescription({
  ...
  logger.debug("Agent5i enhancePersonaDescription response", { hasEnhanced: !!enhanced });
...
export async function generatePersonaQuestions({
  ...
  const normalizedPersonaDescription =
    (personaDescription || "").trim() ||
    "No persona description provided";
  ...
  const questionAgentPayload = buildQuestionPayload(
    normalizedDescription,
    normalizedUserGroup,
    normalizedPersonaDescription
  );

  const processAgentPayload =
  buildProcessDiscoveryPayload(
    projectName,
    normalizedDescription
  );

  console.log(
  "Process Agent Payload:",
  JSON.stringify(processAgentPayload, null, 2)
);

  const [questionAgentResponse, processAgentResponse] =
    await Promise.all([
      callAgent(questionAgentPayload),
      callAgent(processAgentPayload),
    ]);
    console.log(
  "Process Agent Response:",
  JSON.stringify(processAgentResponse, null, 2)
);

 const questionAgentQuestions =
  extractQuestions(questionAgentResponse);

const processDiscoveryQuestions =
  extractMandatoryQuestions(processAgentResponse);

if (
  !questionAgentQuestions.length &&
  !processDiscoveryQuestions.length
) {
  throw new Error("No questions were generated by the agents");
}
