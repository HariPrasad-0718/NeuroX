...
export const problemStructuringSchema = z.object({
  empathy_data_and_context: z
    .string()
    .min(1, "empathy_data_and_context is required")
    .max(30000)
...
export const generateBRDSchema = z.object({
  ...
  businessOwner: z.string().max(150).trim().optional().default(""),
  productOwner: z.string().max(150).trim().optional().default(""),
  engineeringLead: z.string().max(150).trim().optional().default(""),
  complianceOwner: z.string().max(150).trim().optional().default(""),
  endUsers: z.string().max(1000).trim().optional().default(""),
  budgetRange: z.string().max(150).trim().optional().default(""),
  expectedTimeline: z.string().max(150).trim().optional().default(""),
  regulatoryRequirements: z.string().max(4000).trim().optional().default(""),
});

export const updateBRDSchema = z.object({
  projectId: positiveInt,
  brdDocument: z.record(z.unknown()).optional(),
...
export const updateProgressSchema = z.object({
  stage: z.enum(["empathize", "define", "ideate"], {
